...
class Dither:
    ...
    def pil2numpy(self, image=None):
        ...
        self.pil2numpy_matrix = numpy.asarray(image, dtype=numpy.float)
    ...
    def ordered_dither(self, image_matrix, palette_name, map_to_use):
        ...
        for y in range(rows):
            for x in range(cols):
                ...
                old_pixel += old_pixel * self.ordered_diffusion_matrices[map_to_use][x % map_size][y % map_size]
                    ...
                    if y + di + 1 < rows:
                        ...
                        for ci, coeff in enumerate(downward_diffusion):
                            if 0 <= x + ci - offset < cols:
                                new_matrix[x + ci - int(offset)][y + di + 1] += quant_error * coeff
